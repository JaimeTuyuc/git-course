...
function duplicateCount(text: string): number {
  return [...new Set(text.toLowerCase().split("").sort())].reduce(
    (acc: number, letter: string, idx: number, currArr: string[]) => {
      const countRegex = new RegExp(currArr[idx], "g");
      const currentMatch = text.toLowerCase().match(countRegex);
